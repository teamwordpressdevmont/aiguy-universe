...
@section('content')
    ...
    @endif
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('blog.categories.addEdit') }}" class="btn bg-primary/25 text-primary hover:bg-primary hover:text-white">
                Add New
            </a>
        </div>
        ...
        <div class="relative overflow-x-auto">
            <div class="grid lg:grid-cols-3 gap-6">
                <form id="searchForm" method="GET" action="{{ route('blog.categories.list') }}" class="relative w-full max-w-96 mb-5 flex">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="text-xs text-gray-700 bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">
                                <a href="{{ route('blog.categories.list', array_merge(request()->all(), ['sort_by' => 'name', 'sort_direction' => request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}">
                                    Name
                                </a>
                            </th>
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">
                    ...
                    @foreach($categories as $index => $category)
                        <tr class="bg-white border-b border-gray-200">
                            ...
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
                                @if($category->icon)
                                    <img src="{{ asset('public/storage/blog-category-images/' . $category->icon) }}" alt="Logo" width="50">
                                @else
                                    No Icon
                                @endif
                            </td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
                                <div class="flex gap-4">
                                    ...
                                    <a href="{{ route('blog.category.delete', $category->id) }}" onclick="return confirm('Are you sure you want to delete this blog?');">
