...
@endif
    ...
    <h1 class="text-3xl font-bold tracking-tight text-gray-900 mb-6"> {{ isset($category) ? 'Update Blog Category' : 'Add Blog Category' }}</h1>
    <form id="blogCategory" action="{{ isset($category) ? route('blog.categories.update', $category->id) : route('blog.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        ...
        <div class="grid lg:grid-cols-2 gap-6">
            ...
            <div class="">
                ...
                <div class="mt-2">
                    <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-green-600">
                        <input type="text" name="name" id="name" class="form-input" placeholder="Category Name" value="{{ isset($category) ? $category->name : '' }}"> <!-- Pre-fill if editing -->
                    </div>
                </div>
            </div>
            {{--  Slug Field  --}}
            <div class="">
                ...
                <div class="mt-2">
                    <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-green-600">
                        <input type="text" name="slug" id="slug" class="form-input" placeholder="category slug" value="{{ old('slug', $category->slug ?? '') }}">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="sm:col-span-4 mb-5">
            ...
            <div class="mt-2">
                <div class="flex items-center rounded-md bg-white  outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-green-600">
                    ...
                    class="textarea_editor form-input" cols="30" rows="10">{{ isset($category) ? $category->description : '' }}</textarea> <!-- Pre-fill if editing -->
                </div>
            </div>
        </div>
            ...
            <div class="mt-2 grid grid-cols-1">
                <input type="file" name="icon" accept="image/*" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    ...
                    <div id="PreviewContainer" class="mt-2  relative">
                        <img  src="{{ asset('public/storage/blog-category-images/' . $category->icon) }}" class="Preview w-32 h-32 object-cover rounded-lg border border-gray-300">
