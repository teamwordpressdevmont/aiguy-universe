...
@endif
            ...
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 mb-6"> {{ isset($category) ? 'Update AI Tool Category' : 'Add AI Tool Category' }}</h1>



                <form id="aiToolCategory" action="{{ isset($category) ? route('tools.categories.update', $category->id) : route('tools.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                        ...
                        <div class="mt-2">
                            <div class="flex items-center rounded-md bg-white  outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-green-600">
                                <textarea name="description" id="description"
                                          class="textarea_editor form-input"
                        ...
                        <div class="mt-2 grid grid-cols-1">
                            ...
                            @if(isset($category) && $category->icon)
                                <div id="PreviewContainer" class="mt-2  relative">
                                    <img  src="{{ asset('public/storage/ai-tools-category-images/' . $category->icon) }}" class="Preview w-32 h-32 object-cover rounded-lg border border-gray-300">
