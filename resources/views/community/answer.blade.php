...
@section('content')
    ...
    @endif
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900">Community Answer</h1>
        </div>
        <div class="relative overflow-x-auto">
            <div class="grid lg:grid-cols-3 gap-6">
            <form id="searchForm" method="GET" action="{{ route('community.answer') }}" class="relative w-full max-w-96 mb-5 flex">
            ...
            <div id="table-container" class="border rounded-lg overflow-x-auto ">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 text-left">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">
                                <a href="{{ route('community.answer', array_merge(request()->all(), ['sort_by' => 'comment', 'sort_direction' => request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}">
                                    Answer
                                </a>
                            </th>
                    ...
                    @else      
                    @foreach($CommunityAnswers as $index => $CommunityAnswer)
                        <tr class="bg-white border-b border-gray-200">
                             <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap text-start">
                                {{ ($CommunityAnswers->currentPage() - 1) * $CommunityAnswers->perPage() + $index + 1 }}
                            </th>
                            <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap text-start">
                                {{ $CommunityAnswer->id }}
                            </th>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommunityAnswer->user->full_name }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommunityAnswer->aiToolCategory->name }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommunityAnswer->question->question_title }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommunityAnswer->answer }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommunityAnswer->like_count }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommunityAnswer->dislike_count }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
                                <div class="flex gap-4">
                                    <form action="{{ route('community.updateStatusAnswer', $CommunityAnswer->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" name="approved" value="{{ $CommunityAnswer->approved == 1 ? 0 : 1 }}" 
                                            class="px-3 py-1 rounded text-white 
                                            {{ $CommunityAnswer->approved == 1 ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700' }}">
                                            {{ $CommunityAnswer->approved == 1 ? 'Disapprove' : 'Approve' }}
                                        </button>
                                    </form>
                                </div>
                            </td>                
            ...
            <div class="mt-4 pagination-container flex" id="pagination-container">
                ...
                {!! $CommunityAnswers->appends(request()->query())->links() !!}
            </div>
        </div>
