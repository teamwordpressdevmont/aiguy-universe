...
@section('content')
    ...
    @endif
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900">Question List</h1>
        </div>
        <div class="relative overflow-x-auto">
            <div class="grid lg:grid-cols-3 gap-6">
            <form id="searchForm" method="GET" action="{{ route('question-answer.questions-list') }}" class="relative w-full max-w-96 mb-5 flex">
            ...
            <div id="table-container" class="border rounded-lg overflow-x-auto ">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 text-left">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">
                                <a href="{{ route('question-answer.questions-list', array_merge(request()->all(), ['sort_by' => 'comment_title', 'sort_direction' => request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}">
                                    Question Title
                                </a>
                            </th>
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">Question Content</th>
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500" width="130">Action</th>                       
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">     
                    @if($CommentQuestions->isEmpty())
                    ...
                    @else      
                    @foreach($CommentQuestions as $index => $CommentQuestion)
                        <tr class="bg-white border-b border-gray-200">
                             <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap text-start">
                                {{ ($CommentQuestions->currentPage() - 1) * $CommentQuestions->perPage() + $index + 1 }}
                            </th>
                            <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap text-start">
                                {{ $CommentQuestion->id }}
                            </th>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommentQuestion->user->full_name }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommentQuestion->tool->name }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommentQuestion->comment_title }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $CommentQuestion->comment_content }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
                                <div class="flex gap-4">
                                    <form action="{{ route('question-answer.updateStatus', $CommentQuestion->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" name="approved" value="{{ $CommentQuestion->approved == 1 ? 0 : 1 }}" 
                                            class="px-3 py-1 rounded text-white 
                                            {{ $CommentQuestion->approved == 1 ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700' }}">
                                            {{ $CommentQuestion->approved == 1 ? 'Disapprove' : 'Approve' }}
                                        </button>
                                    </form>
                                    <a href="{{ route('question-answer.question-view', $CommentQuestion->id) }}">
            ...
            <div class="mt-4 pagination-container flex" id="pagination-container">
                ...
                {!! $CommentQuestions->appends(request()->query())->links() !!}
            </div>
        </div>
