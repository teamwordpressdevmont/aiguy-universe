...
@section('content')
    ...
    @endif
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900">User List</h1>
        </div>
        <div class="relative overflow-x-auto">
            <div class="grid lg:grid-cols-3 gap-6">
            <form id="searchForm" method="GET" action="{{ route('user.list') }}" class="relative w-full max-w-96 mb-5 flex">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 text-left">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">User Pic</th>
                    ...
                    @foreach($user as $index => $users)
                        <tr class="bg-white border-b border-gray-200">
                            ...
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $users->full_name }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $users->email }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
                                @if($users->avatar)
                                    <img src="{{  $users->avatar }}" alt="avatar" width="50">
                                @else
                                    No Avatar
                                @endif
                            </td>
