                    ...
                    <thead class="bg-gray-50 text-left">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">
                                <a href="{{ route('collection.list', array_merge(request()->all(), ['sort_by' => 'folder_name', 'sort_direction' => request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}">
                                  Collection  Name
                                </a>
                            </th>
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">User Name</th>
                            <th scope="col" class="px-6 py-3 text-start text-sm text-default-500">Total Tools</th>
                    ...
                    @foreach($collection as $index => $collections)
                        <tr class="bg-white border-b border-gray-200">
                             <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap text-start">
                                {{ ($collection->currentPage() - 1) * $collection->perPage() + $index + 1 }}
                            </th>
                            <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap text-start">
                                {{ $collections->id }}
                            </th>
                            <td class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $collections->folder_name }}</td>
